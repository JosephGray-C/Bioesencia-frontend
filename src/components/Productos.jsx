...
export default function Productos() {
  ...
  const cargarProductos = async () => {
    try {
      ...
      // 👇 aquí filtras según tu modelo
      const activos = lista.filter(
        (p) => (p.activo === true || p.estado === "ACTIVO") && p.stock > 0
      );
  ...
  const handleAgregarCarrito = async () => {
    ...
    try {
      const res = await fetch(
        `${API_CARRITO}?usuarioId=${user.id}&productoId=${modal.id}&cantidad=${cantidad}`,
        {
          method: "POST",
          credentials: "include"
        }
      ...
      if (!res.ok) {
        const errText = await res.text();
        throw new Error(errText || "Error al agregar al carrito");
      }
          ...
          productos.map(
            (prod) =>
              prod && (
                ...
                >
                  <img
                    src={prod.imagenUrl || "/placeholder.jpg"}
                    alt={prod.nombre}
                  />
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <img
              src={modal.imagenUrl || "/placeholder.jpg"}
              alt={modal.nombre}
            />
              ...
              <input
                ...
                onChange={(e) =>
                  setCantidad(
                    Math.max(
                      1,
                      Math.min(modal.stock, Number(e.target.value) || 1)
                    )
                  )
                }
                min={1}
