...
async function postAgregarCarrito({ userId, productoId, cantidad }) {
  const url = `${API_CARRITO}?usuarioId=${encodeURIComponent(userId)}&productoId=${encodeURIComponent(productoId)}&cantidad=${encodeURIComponent(cantidad)}`;
...
function ProductoModal({ producto, userId, onClose }) {
  ...
  const mAgregar = useMutation({
    ...
    onMutate: async ({ userId: uid, cantidad: cant, producto: p }) => {
      Swal.fire({ icon: "success", title: "Producto agregado", text: `${p.nombre} fue agregado al carrito.`, timer: 1100, showConfirmButton: false });
      const key = ["carrito", uid];
      await qc.cancelQueries({ queryKey: key });
      const prev = qc.getQueryData(key) || [];
      const existente = prev.find((it) => it?.producto?.id === p.id);
      const siguiente = existente
        ? prev.map((it) => (it.producto.id === p.id ? { ...it, cantidad: Number(it.cantidad || 0) + Number(cant || 0) } : it))
        : [...prev, { id: `temp-${p.id}-${Date.now()}`, cantidad: Number(cant || 1), producto: { id: p.id, nombre: p.nombre, precio: Number(p.precio || 0) } }];
  ...
  const handleAgregar = () => {
    ...
    mAgregar.mutate({ userId, productoId: producto.id, cantidad, producto: { id: producto.id, nombre: producto.nombre, precio: producto.precio } });
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-price">
          {Number(producto.precio || 0).toLocaleString("es-CR", { style: "currency", currency: "CRC", minimumFractionDigits: 2 })}
        </div>
...
export default function Productos() {
  ...
  const { data: productos = [], isFetching } = useQuery({
    ...
    initialData: () => qc.getQueryData(["productosActivos", location.pathname]) || [],
    onError: () => { Swal.fire("Error", "No se pudieron cargar los productos", "error"); },
  });
      ...
      <div className="productos-grid">
        {productos.length === 0 ? (
          <div
            ...
            style={{ gridColumn: "1 / -1", minHeight: 180, display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            {showSpinner ? (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
                <ClipLoader size={24} color="#A9C499" speedMultiplier={0.9} />
                <span style={{ color: "#5A0D0D" }}>Cargando productos…</span>
              </span>
            ) : (
              ...
              prod && (
                <div
                  ...
                  <div className="producto-precio">
                    {Number(prod.precio || 0).toLocaleString("es-CR", { style: "currency", currency: "CRC", minimumFractionDigits: 2 })}
                  </div>
                </div>
              )
          )
